...
const {
    errors, respuesta, agregarPopup, actualizarPopup, listaPopups
} = usePopup();
const  emit  =defineEmits(['onListar'])
const crud = {
    'nuevo': async() => {
        await agregarPopup(form.value)
        form.value.errors = []
        if(errors.value)
        {
    ...
    'editar': async() => {
        await actualizarPopup(form.value)
        form.value.errors = []
        if(errors.value)
        {
            form.value.errors = errors.value
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="modalpopupLabel">Modal title</h1>
