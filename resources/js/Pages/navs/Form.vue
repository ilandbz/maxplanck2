...
const {
    errors, respuesta, agregarNav, actualizarNav, listaNavs, navs
} = useNav();
const  emit  =defineEmits(['onListar'])
const crud = {
    'nuevo': async() => {
        await agregarNav(form.value)
        form.value.errors = []
        if(errors.value)
        {
    ...
    'editar': async() => {
        await actualizarNav(form.value)
        form.value.errors = []
        if(errors.value)
        {
            form.value.errors = errors.value
...
<template>
    ...
    <div class="modal fade" id="modalnav" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        aria-labelledby="modalnavLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="modalnavLabel">Modal title</h1>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="name" class="form-label">Nav Padre</label>
                        <select v-model="form.padre_id" class="form-control"
                            :class="{ 'is-invalid': form.errors.padre_id }">
                            <option value="">--Seleccione--</option>
                            <option v-for="nav in navs" :key="nav.id" :value="nav.id"
                                :title="nav.nombre">
                                {{ nav.nombre }}
                            </option>
                        </select>
